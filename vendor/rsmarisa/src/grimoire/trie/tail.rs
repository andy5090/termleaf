...
impl Tail {
    ...
    pub fn prefix_match(&self, agent: &mut crate::agent::Agent, offset: usize) -> bool {
        ...
        if self.end_flags.empty() {
            // Text mode
            // The C++ implementation forms a pointer before `buf_[offset]`
            // and indexes it with the absolute query position. Retain the
            // initial query position so the equivalent Rust index never
            // underflows.
            let initial_query_pos = query_pos;
            loop {
                let buf_index = offset + (query_pos - initial_query_pos);
